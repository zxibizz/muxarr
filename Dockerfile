...
# interpreter that built it, and a python-only bump once shipped a venv whose
# site-packages the new interpreter never looked at.
...
# --build-arg INCLUDE_FFMPEG=true if you want `inspect` to handle them.
...
# A mux can take hours. Give it room to finish rather than SIGKILLing mid-write.
