...
#
# muxarr import shim for Sonarr.
#
# Set as: Settings > Media Management > Importing > Import Using Script
#         "Import Script Path" -> /path/to/muxarr-import-sonarr.sh
#
# Sonarr calls this as:  script "<sourcePath>" "<destinationPath>"
...
#
# Exit codes are load-bearing. Sonarr FAILS the import on a non-zero exit, and an
# unrecognised/missing [MoveStatus] line makes it assume the file was already
# moved. So:
#   * nothing queued yet -> exit 0 with [MoveStatus] DeferMove, and Sonarr
...
#   MUXARR_API_KEY        from Muxarr's Settings > Security (or its MUXARR_API_KEY)
...
# No `set -u`: every Sonarr variable is referenced with an explicit default anyway.

MUXARR_URL="${MUXARR_URL:-http://muxarr:8710}"
MUXARR_API_KEY="${MUXARR_API_KEY:-}"
...
# Nothing has been queued: let Sonarr do a normal import.
defer() {
    if [ -n "$1" ]; then
        log "$1"
    fi
...
# Sonarr move a file muxarr may still be rewriting.
fail() {
    log "$1"
    exit 1
}
...
# For the informational fields: a control character would make the JSON invalid
# and cost the whole import a minute of retries.
json_text() {
    json_escape "$1" | tr -d '\001-\037'
}
...
http_post() {
    ...
    if [ "$HTTP_CLIENT" = curl ]; then
        ...
        [ -n "$MUXARR_API_KEY" ] && set -- "$@" -H "X-Api-Key: $MUXARR_API_KEY"
        curl "$@" --data "$_payload" "$_endpoint" 2>/dev/null
    else
        set -- -q -O - --timeout="$REQUEST_TIMEOUT" \
            --header='Content-Type: application/json'
        [ -n "$MUXARR_API_KEY" ] && set -- "$@" --header="X-Api-Key: $MUXARR_API_KEY"
...
SOURCE="${1:-}"
DESTINATION="${2:-}"
# Sonarr passes its variables through a .NET StringDictionary, which lower-cases
# every name; the mixed-case spelling is the documented one, kept as a fallback.
TRANSFER="${sonarr_transfermode:-${Sonarr_TransferMode:-Move}}"
...
ARR=$(printf '{"instance":"%s","url":"%s","title":"%s","year":"%s","slug":"%s","original_language":"%s","tags":"%s"}' \
    "$(json_text "${sonarr_instancename:-}")" \
    "$(json_text "${sonarr_applicationurl:-}")" \
    "$(json_text "${sonarr_series_title:-}")" \
    "$(json_text "${sonarr_series_year:-}")" \
    "$(json_text "${sonarr_series_titleslug:-}")" \
    "$(json_text "${sonarr_series_originallanguage:-}")" \
    "$(json_text "${sonarr_series_tags:-}")")

PAYLOAD=$(printf '{"job_id":"%s","app":"sonarr","source_path":"%s","destination_path":"%s","transfer_mode":"%s","arr":%s}' \
    "$JOB_ID" \
    "$(json_escape "$SOURCE")" \
...
while :; do
    ...
    case "$STATE" in
        ...
        # page or a stray log line can never reach Sonarr's parser.
        LINES=$(printf '%s\n' "$RESPONSE" | tr -d '\r' \
