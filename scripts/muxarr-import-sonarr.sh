...
# unrecognised/missing [MoveStatus] line makes Sonarr assume the file was already
...
PAYLOAD=$(printf '{"app":"sonarr","source_path":"%s","destination_path":"%s","transfer_mode":"%s"}' \
    "$(json_escape "$SOURCE")" \
...
# can never reach Sonarr's parser.
LINES=$(printf '%s\n' "$RESPONSE" \
    | tr -d '\r' \
