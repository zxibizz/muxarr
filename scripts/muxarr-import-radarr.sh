...
SOURCE="${1:-}"
DESTINATION="${2:-}"
# Radarr passes its variables through a .NET StringDictionary, which lower-cases
# every name; the mixed-case spelling is the documented one, kept as a fallback.
TRANSFER="${radarr_transfermode:-${Radarr_TransferMode:-Move}}"
...
ARR=$(printf '{"instance":"%s","url":"%s","title":"%s","year":"%s","slug":"%s","original_language":"%s","tags":"%s"}' \
    "$(json_text "${radarr_instancename:-}")" \
    "$(json_text "${radarr_applicationurl:-}")" \
    "$(json_text "${radarr_movie_title:-}")" \
    "$(json_text "${radarr_movie_year:-}")" \
    "$(json_text "${radarr_movie_tmdbid:-}")" \
    "$(json_text "${radarr_movie_originallanguage:-}")" \
    "$(json_text "${radarr_movie_tags:-}")")

PAYLOAD=$(printf '{"job_id":"%s","app":"radarr","source_path":"%s","destination_path":"%s","transfer_mode":"%s","arr":%s}' \
    "$JOB_ID" \
    "$(json_escape "$SOURCE")" \
